...
def evaluate(params, net, dataset, segment='valid'):
    ...
    net.eval()
    with th.no_grad():
        pred_ratings, neg_rating, _, _, _ = net(enc_graph, dec_graph, dec_graph,
...
def train(params):
    ...
    params.src_in_units = dataset.user_feature_shape[1] #num_user
    params.dst_in_units = dataset.movie_feature_shape[1] #num_movie
    params.rating_vals = dataset.possible_rating_values #가능한 평점 종류(ex. 1~5)
    ...
    for iter_idx in range(1, params.train_max_iter):
        ...
        pred_ratings2, neg_rating2, ed_mi2, user2, item2 = net(dataset.train_enc_graph,
                                                  dataset.train_dec_graph,
                                                  neg_dec_graph,
                                                  dataset.user_feature,
                                                  dataset.movie_feature)
        loss1 = rating_loss_net(pos_prediction, train_gt_labels).mean()
        ...
        logging_str = f"Iter={iter_idx:>4d}, Train_RMSE={train_rmse:.4f}, ED_MI={ed_loss:.4f}, ND_MI={nd_loss:.4f}, " \
                      f"Valid_RMSE={valid_rmse:.4f}, Train_BPR_Loss={bpr_loss:.4f},"
